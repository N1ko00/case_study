...
public class InventoryToggle : MonoBehaviour
{
    ...
    [Header("インベントリ本体（InventoryPanel_1）を割当")]
    public GameObject inventoryUI;

    [Header("背景に表示したい画像（inventorybackground）を割当")]
    public GameObject inventoryBG;
    ...
    void Update()
    {
        ...
        // ★ゲームパッドのYボタン、またはセレクトボタン(Viewボタン)のどちらでも開閉できるように設定
        bool gamepadYPressed = Gamepad.current != null && (Gamepad.current.buttonNorth.wasPressedThisFrame || Gamepad.current.selectButton.wasPressedThisFrame);

        if (tabPressed || gamepadYPressed)
        {
            if (keypadUI != null && keypadUI.activeSelf) return;
            if (GameOverManager.Instance != null && GameOverManager.Instance.IsGameOver) return;
            if (PauseMenuManager.Instance != null && PauseMenuManager.Instance.IsOpen) return;
            if (cameraSwitcher != null && cameraSwitcher.CurrentCameraIndex != 0) return;
    ...
    public void OpenInventory()
    {
        ...
        if (UIInventory.Instance != null)
        {
            UIInventory.Instance.OpenInventory(); // UIInventory側のOpen処理へ統合
        }
        Debug.Log("インベントリを開きました");
    }
